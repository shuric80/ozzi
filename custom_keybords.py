...
def keyboard_menu_settings(d_input):
    ...
    button = types.InlineKeyboardButton(text='Choose groups',
...
def keyboard_settings(d_input):
    ...
    for btn in d_input['groups']:
        ...
        callable_button = types.InlineKeyboardButton(text=btn_title,
                                                     callback_data=json.dumps(
                                                         dict(id=d_input['id'],
                                                              button=btn.id)))
...
def keyboard_list_groups(q, id):
    ...
    for btn in q:
        ...
        callable_button = types.InlineKeyboardButton(
            text=name[:50] if name else 'Noname',
...
def keyboard_view_info(url):
    ...
    url_button = types.InlineKeyboardButton(
        text='Group', url='http://vk.com/{url}'.format(url=url))
...
def keyboard_last_posts(post, id):
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    callable_button = types.InlineKeyboardButton(text='\U00002934',
                                                 callback_data=json.dumps(
                                                     dict(id=id,
                                                          button=post.id)))
    info_button = types.InlineKeyboardButton(text='\U00002139',
                                             callback_data=json.dumps(
                                                 dict(group=group_id,
                                                      button='INFO')))
    url_button = types.InlineKeyboardButton(
        text='Group', url='http://vk.com/{url}'.format(url=post.group.url))
...
def keyboard_next_page(group, sid):
    ...
    for i in ['\U000023EA', '\U000023E9']:
        btn = types.InlineKeyboardButton(text=str(i),
                                         callback_data=json.dumps(
                                             dict(id=sid, button=i.upper())))
        keys.append(btn)

    #group = db.get_group(group_id)
    #keys.append(types.InlineKeyboardButton(text='Group', url='http://vk.com/%s'%group.url))
    keys.append(
        types.InlineKeyboardButton(text='\U00002139',
                                   callback_data=json.dumps(
                                       dict(group=group.id, button='INFO'))))
    keyboard.add(*keys)
